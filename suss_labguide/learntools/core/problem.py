...
import json
import builtins
from learntools.vli.problemx import ProblemX
...
class CodingProblem(ProblemX):
    def print_class_name(self):
        class_file = inspect.getfile(self.__class__)
        return self.__class__.__name__,  os.path.abspath(class_file)
...
# similar to FunctionProblem but meant for CodingProblem with `OOP` or class-based solution.
class ClassProblem(CodingProblem):
    # Must have a single _var, corresponding to a function name
    ...
    @classmethod
    def check_whether_attempted(cls, fn):
        # Not sure if inspect.getsource() will work reliably? 
        # Seems like it does
        # work okay with ipython notebooks. 
        def dummy(): 
            pass
...
__all__ = ['Problem', 'EqualityCheckProblem', 'FunctionProblem',
        'ThoughtExperiment', 'CodingProblem', 'ClassProblem'
        ]
